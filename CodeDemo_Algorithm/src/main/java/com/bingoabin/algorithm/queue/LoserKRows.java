...
public class LoserKRows {
	...
	//     方式三：使用小根堆，将所有数据放入到小根堆中，然后进行取出k个即可
	public static void main(String[] args) {
		...
		System.out.println(Arrays.toString(kWeakestRows3(mat, 3)));
		//验证统计军人数量
		count(mat);
	}
	...
	public static int[] kWeakestRows2(int[][] mat, int k) {
		...
		for (int i = 0; i < row; i++) {
			...
			if (queue.size() >= k && queue.peek()[1] > pos) {
				queue.poll();
			}
			if (queue.size() < k) queue.offer(new int[]{i, pos});
		}
		//求出结果集，弹出
		int[] res = new int[k];
		int inx = k - 1;
