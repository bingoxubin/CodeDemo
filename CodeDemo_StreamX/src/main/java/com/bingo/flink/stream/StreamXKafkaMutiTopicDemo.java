 ...
 * @Description:
 */
public class StreamXKafkaMutiTopicDemo {
	public static void main(String[] args) {
		...
		// 消费 kafka 数据
		DataStream<String> source1 = new KafkaSource<String>(ctx)
				.topic("s1")   //也可以在这里用逗号隔开，添加多个
				.getDataStream()
		...
		// 消费 kafka 数据
		DataStream<String> source2 = new KafkaSource<String>(ctx)
				.topic("s2")
				.getDataStream()
				.map(new MapFunction<KafkaRecord<String>, String>() {
					@Override
		...
		// 消费 kafka 数据
		DataStream<String> source3 = new KafkaSource<String>(ctx)
				.topic("s3")
				.getDataStream()
				.map(new MapFunction<KafkaRecord<String>, String>() {
					@Override
