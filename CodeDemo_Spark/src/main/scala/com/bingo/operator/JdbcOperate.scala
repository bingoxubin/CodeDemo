...
object JdbcOperate {
	...
	val getConn: () => Connection = () => {
		DriverManager.getConnection("jdbc:mysql://localhost:3306/mydb?characterEncoding=UTF-8", "root", "123456")
	}
	...
	def main(args: Array[String]): Unit = {
		...
		rsultRdd2.foreachPartition(iter => { //创建数据库连接
			val conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/mydb?characterEncoding=UTF-8", "root", "123456")
			//关闭自动提交
			conn.setAutoCommit(false);
			val statement = conn.prepareStatement(s"insert into job_count(search_name, job_num) values (?, ?)")
			//遍历
			iter.foreach(record => {
				//赋值操作
		...
		details.foreach(x => {
			println(x.job_id + "\t" + x.job_salary + "\t" + x.job_company)
		})
		sparkContext.stop()
	}
}
